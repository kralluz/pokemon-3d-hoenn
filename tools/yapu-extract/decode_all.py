...
LAYOUTS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "layouts_all.json")
OUT     = r"C:\Users\Carlos Henrique\OneDrive\Desktop\pokemon\YAPU_Demo\extracted\MonoBehaviour_decoded"

J        = json.load(open(LAYOUTS, encoding="utf-8"))
LAYOUTS_BY_CLASS = {k.split(".")[-1]: v for k, v in J["layouts"].items() if v}
REFTYPES = J["refTypes"]
...
for src, env in envs.items():
    for o in env.objects:
        ...
        try:
            ...
            continue
        layout = LAYOUTS_BY_CLASS.get(cls)
        ...
        try:
            ...
            stats["falha"] += 1; failclass[cls]+=1; problems.append((src, cls+"/"+str(o.path_id), repr(e)[:120])); continue

        left = len(raw) - r.p
        if left != 0:
            stats["bytes sobrando"] += 1; failclass[cls]+=1
            problems.append((src, cls+"/"+name, f"{left} de {len(raw)} bytes nao consumidos")); continue

        stats["ok"] += 1
        obj["$class"] = cls
        results.setdefault(cls, {})[name] = obj
        folder = os.path.join(OUT, cls)
        os.makedirs(folder, exist_ok=True)
        safe = "".join(ch if ch.isalnum() or ch in "._- " else "_" for ch in name)[:100] or str(o.path_id)
        json.dump(obj, open(os.path.join(folder, safe + ".json"), "w", encoding="utf-8"),
                  indent=1, ensure_ascii=False)

json.dump(results, open(os.path.join(OUT, "_all.json"), "w", encoding="utf-8"),
          indent=1, ensure_ascii=False)
print("\n=== RESULTADO ===")
for k, v in stats.most_common(): print(f"  {k}: {v}")
print("--- detalhe ---")
for pr in problems: print("  !", " | ".join(pr))
